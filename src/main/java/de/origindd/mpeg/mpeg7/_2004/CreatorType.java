//
// This file was generated by the JavaTM Architecture for XML Binding(JAXB) Reference Implementation, v2.2.4-2 
// See <a href="http://java.sun.com/xml/jaxb">http://java.sun.com/xml/jaxb</a> 
// Any modifications to this file will be lost upon recompilation of the source schema. 
// Generated on: 2015.01.12 at 06:39:36 PM BRST 
//


package de.origindd.mpeg.mpeg7._2004;

import lombok.Data;

import javax.xml.bind.annotation.XmlAccessType;
import javax.xml.bind.annotation.XmlAccessorType;
import javax.xml.bind.annotation.XmlElement;
import javax.xml.bind.annotation.XmlSeeAlso;
import javax.xml.bind.annotation.XmlType;
import java.util.ArrayList;
import java.util.List;


/**
 * <p>Java class for CreatorType complex type.
 * 
 * <p>The following schema fragment specifies the expected content contained within this class.
 * 
 * <pre>
 * &lt;complexType name="CreatorType">
 *   &lt;complexContent>
 *     &lt;extension base="{urn:mpeg:mpeg7:schema:2004}MediaAgentType">
 *       &lt;sequence>
 *         &lt;element name="Character" type="{urn:mpeg:mpeg7:schema:2004}PersonNameType" maxOccurs="unbounded" minOccurs="0"/>
 *         &lt;element name="Instrument" type="{urn:mpeg:mpeg7:schema:2004}CreationToolType" maxOccurs="unbounded" minOccurs="0"/>
 *       &lt;/sequence>
 *     &lt;/extension>
 *   &lt;/complexContent>
 * &lt;/complexType>
 * </pre>
 * 
 * 
 */
@XmlAccessorType(XmlAccessType.FIELD)
@XmlType(name = "CreatorType", propOrder = {
    "character",
    "instrument"
})
@XmlSeeAlso({
    CreationPreferencesType.Creator.class
})
@Data
public class CreatorType
    extends MediaAgentType
{

    @XmlElement(name = "Character")
    protected List<PersonNameType> character;
    @XmlElement(name = "Instrument")
    protected List<CreationToolType> instrument;

    /**
     * Gets the value of the character property.
     * 
     * <p>
     * This accessor method returns a reference to the live list,
     * not a snapshot. Therefore any modification you make to the
     * returned list will be present inside the JAXB object.
     * This is why there is not a <CODE>set</CODE> method for the character property.
     * 
     * <p>
     * For example, to add a new item, do as follows:
     * <pre>
     *    getCharacter().add(newItem);
     * </pre>
     * 
     * 
     * <p>
     * Objects of the following type(s) are allowed in the list
     * {@link PersonNameType }
     * 
     * 
     */
    public List<PersonNameType> getCharacter() {
        if (character == null) {
            character = new ArrayList<PersonNameType>();
        }
        return this.character;
    }

    public List<PersonNameType> addCharacter(PersonNameType character) {
        getCharacter().add(character);
        return getCharacter();
    }

    /**
     * Gets the value of the instrument property.
     * 
     * <p>
     * This accessor method returns a reference to the live list,
     * not a snapshot. Therefore any modification you make to the
     * returned list will be present inside the JAXB object.
     * This is why there is not a <CODE>set</CODE> method for the instrument property.
     * 
     * <p>
     * For example, to add a new item, do as follows:
     * <pre>
     *    getInstrument().add(newItem);
     * </pre>
     * 
     * 
     * <p>
     * Objects of the following type(s) are allowed in the list
     * {@link CreationToolType }
     * 
     * 
     */
    public List<CreationToolType> getInstrument() {
        if (instrument == null) {
            instrument = new ArrayList<CreationToolType>();
        }
        return this.instrument;
    }

    public List<CreationToolType> addInstrument(CreationToolType instrument) {
        getInstrument().add(instrument);
        return getInstrument();
    }

}
